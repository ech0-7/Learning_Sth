...
def horizon_concate(inp0, inp1):#俩个图像横向连接放在一起方便观察
    h0, w0 = inp0.shape[:2]
    h1, w1 = inp1.shape[:2]
    if inp0.ndim == 3:
...
colors = color_list.reshape((-1, 3)) * 255#并将颜色值从 [0, 1] 范围转换到 [0, 255] 范围
colors = np.array(colors, dtype=np.uint8).reshape(len(colors), 1, 1, 3)#(80,1,1,3)
